...
export default function Welcome({ onTimelineDialogRequest }: WelcomeProps) {
  ...
  const typeNextOutputLine = useCallback(() => {
    ...
    const delay = line.type === 'code' ? 120 : line.text ? 420 : 220
  ...
  const typeLongPromptChar = useCallback(() => {
    ...
    setTimeout(() => terminalScrollRef.current?.scrollTo({ top: terminalScrollRef.current?.scrollHeight ?? 0, behavior: 'smooth' }), 0)
    animationTimeoutRef.current = setTimeout(typeLongPromptChar, 45 + Math.random() * 25)
  ...
  const typePromptChar = useCallback(() => {
    ...
    setTimeout(() => terminalScrollRef.current?.scrollTo({ top: terminalScrollRef.current?.scrollHeight ?? 0, behavior: 'smooth' }), 0)
    animationTimeoutRef.current = setTimeout(typePromptChar, 75 + Math.random() * 35)
  ...
  const typeReviseChar = useCallback(() => {
    ...
    reviseTimeoutRef.current = setTimeout(typeReviseChar, 70 + Math.random() * 30)
  }, [resumeAfterStep])
  ...
  return (
    <section className="px-4 md:px-0 pt-8 pb-6 md:pt-6 md:pb-8 w-full max-w-5xl mx-auto">
      <div className="text-center mb-8 lg:mb-10">
        ...
        <h2 className="text-2xl sm:text-3xl lg:text-4xl font-semibold text-slate-900 tracking-tight mb-3" id="ai-first-wizard-title">
          <span className="block">Your project, created</span>
          <span className="block text-blue-600">watch the code come to life, step by step</span>
        </h2>
        <p className="text-base lg:text-lg text-gray-600 max-w-3xl mx-auto leading-relaxed">
          Describe the flow once. The agent plans, generates code, runs tests, fixes issues, and prepares deploy — all orchestrated from a single CLI command.
        </p>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-lg border border-gray-200 bg-white p-4 text-left relative shadow-sm">
          ...
          <div className={styles.wizardTerminalWrapper}>
          ...
          <div className="mt-4 pt-4 border-t border-gray-200">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-medium text-emerald-700 border border-emerald-100">
                ...
                {showSuccess ? 'Deployed' : 'Waiting for deploy'}
              </span>
            </div>
            <div className={styles.deploymentTerminal}>
              <div className={styles.deploymentTerminalHeader}>
                <div className={styles.deploymentDots}>
                  <span />
                  <span />
                  <span />
                </div>
                <span className={styles.deploymentTitle}>masterfabric · deploy</span>
                {showSuccess && successCountdown !== null && (
                  <span className={styles.deploymentCountdown}>
                    Auto-restart in {successCountdown}s
                  </span>
                )}
              </div>
              <div className={styles.deploymentTerminalBody}>
                <div className={styles.deploymentTerminalLinePrimary}>$ masterfabric deploy</div>
                {showSuccess && (
                  <>
                    <div className={styles.deploymentTerminalLineSuccess}>✓ validating plugin manifest</div>
                    <div className={styles.deploymentTerminalLineSuccess}>✓ uploading assets to Masterfabric</div>
                    <div className={styles.deploymentTerminalLineSuccess}>✓ publishing stable version</div>
                    <div className={styles.deploymentTerminalLineMuted}>Deploy complete.</div>
                  </>
                )}
              </div>
            </div>
